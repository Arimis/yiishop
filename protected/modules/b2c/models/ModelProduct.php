...
class ModelProduct extends B2cModel
{
    ...
    public function Item($product_id)
    {
        ...
        $row = $this->ModelQueryRow(
            "SELECT g.goods_id,g.name,g.intro,g.cat_id,g.brief,g.image_default_id,p.product_id,p.price,p.cost,p.mktprice,p.spec_desc,p.store,p.freez FROM car_b2c_goods g LEFT JOIN car_b2c_products p ON g.goods_id = p.goods_id
